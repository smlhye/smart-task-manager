...
export const useTasksPending = (groupId: number, filter?: FilterTaskType) => {
    ...
    const { data, fetchNextPage, hasNextPage, isFetchingNextPage } = useInfiniteQuery<CreatedTaskType[], Error>({
        queryKey: ['tasks-pending', filter, groupId],
        queryFn: async ({ pageParam }) => {
            const res = await groupService.getTaskPending(groupId,{
                ...filter,
